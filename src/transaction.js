...
module.exports = class Transaction {
  constructor(amount, reference, installments) {
    if (amount !== undefined) {
      this.amount = amount;
    }
    ...
    for (let keyIndex = keys.length; keyIndex >= 0; keyIndex -= 1) {
      ...
      switch (property) {
        case 'refunds':
          ...
          for (i = 0, t = value.length; i < t; i += 1) {
  ...
  creditCard(cardNumber, securityCode, expirationMonth, expirationYear, cardHolderName) {
    return this.card(
      cardNumber, securityCode, expirationMonth, expirationYear, cardHolderName, Transaction.CREDIT,
    );
  }
  ...
  debitCard(cardNumber, securityCode, expirationMonth, expirationYear, cardHolderName) {
    return this.card(
      cardNumber, securityCode, expirationMonth, expirationYear, cardHolderName, Transaction.DEBIT,
    );
  }
  ...
  autoCapture(capture = true) {
    if (!capture && this.kind === Transaction.DEBIT) {
      throw new Error('Debit transactions will always be captured');
    }
